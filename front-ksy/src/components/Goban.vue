<template>
  <div class="goban-container">
    ...
    >
      ...
      <!-- 影响区域可视化 -->
      <div v-if="showInfluence && influenceMap" class="influence-layer">
        <div
          v-for="(row, x) in influenceMap"
          :key="`influence-row-${x}`"
          class="influence-row"
        >
          <div
            ...
            :style="getInfluenceStyle(x, y, value)"
            :title="`影响值: ${value.toFixed(2)}`"
          ></div>
        </div>
      </div>
...
// 确保aiSuggestions是响应式的
const aiSuggestions = computed(() => gameStore.aiSuggestions)
const showSuggestions = computed(() => gameStore.showSuggestions)
const showInfluence = computed(() => gameStore.showInfluence)
const influenceMap = computed(() => gameStore.influenceMap)
...
const canPlaceStone = (x, y) => {
  // 允许本地模式（未连接AI时也能下棋）
  return gameStore.board[x][y] === 0 && !gameStore.isThinking
}
...
const handleCellClick = async (x, y) => {
  ...
  // 如果显示推荐，自动获取新的推荐
  if (showSuggestions.value && gameStore.isConnected) {
...
const handleCellHover = (x, y) => {
  ...
  // 如果显示影响区域，更新单个棋子的影响
  if (showInfluence.value && gameStore.board[x][y] !== 0) {
    const influence = gameStore.calculateInfluence(x, y)
    if (influence) {
      // 临时显示单个棋子的影响
      // 这里可以添加临时影响显示逻辑
    }
