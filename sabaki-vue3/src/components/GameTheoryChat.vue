<template>
    <div class="game-theory-chart" ref="chartContainerRef">
      ...
      <!-- 胜率曲线 -->
      <div v-if="activeTab === 'winrate'" class="chart-container">
        <div class="chart" ref="winrateChartRef">
            <svg class="chart-svg" :viewBox="`0 0 ${chartWidth.value} ${chartHeight}`" preserveAspectRatio="xMidYMid meet">
            <!-- 背景网格 -->
            <defs>
        ...
        <div class="chart-footer">
          ...
          <div class="chart-stats">
            <div class="stat-mini">
              <span class="stat-mini-label">当前胜率:</span>
              <span class="stat-mini-value" :class="currentWinRate >= 0.5 ? 'black' : 'white'">
      ...
      <!-- 评估值曲线 -->
      <div v-if="activeTab === 'value'" class="chart-container">
      ...
      <!-- 吃子数统计 -->
      <div v-if="activeTab === 'capture'" class="chart-container">
      ...
      <div v-if="activeTab === 'nash'" class="nash-container">
        ...
        <div v-else class="nash-chart">
          <svg class="chart-svg" :viewBox="`0 0 ${chartWidth.value} ${chartHeight}`" preserveAspectRatio="xMidYMid meet">
            ...
            <!-- 轴标签 -->
            <text :x="padding.value - 10" :y="padding + 5" class="axis-label" text-anchor="end">白方策略</text>
            <text :x="chartWidth.value / 2" :y="chartHeight - 10" class="axis-label" text-anchor="middle">黑方策略</text>
      ...
      <div v-if="activeTab === 'pareto'" class="pareto-container">
        ...
        <div v-else class="pareto-chart">
            ...
            <!-- 轴标签 -->
            <text :x="padding.value - 10" :y="padding + 5" class="axis-label" text-anchor="end">黑方收益</text>
  ...
  const getCalculatedQuality = (move) => {
    ...
    const centerQuality = 1 - (centerDist / 18) * 0.3 // 中心区域质量更高
    
    // 基于手数调整（早期手数质量可能较低）
    const moveNum = moveHistory.value.indexOf(move) + 1
    const moveNumFactor = Math.min(1, moveNum / 20) // 前20手质量较低
    
    return Math.max(0.3, Math.min(0.9, centerQuality * 0.7 + moveNumFactor * 0.3))
  }
  ...
  const getNashX = (blackStrategy) => {
    return padding.value + (blackStrategy * (chartWidth.value - 2 * padding.value))
  }
